...
import { addAuditEvent, getEnvelopeById } from "@/lib/store";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return new Response(new Uint8Array(bytes), {
    headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${envelope.title.replace(/[^a-z0-9_-]+/gi, "-")}${suffix}.pdf"`, "Cache-Control": "private, no-store" },
  });
}
